...
class _Router:
    ...
    def route(self, rule, *methods):#用户输入规则转换为pattern
        def wrapper(handler):
            ...
            #(/studnet/(?P<name>[^/]+)/xxx/(?P<id>[+-]?\\d+), [<class 'str'>, <class 'int>])
            ...
            if not methods or request.method.upper() in methods:
                ...
                if matcher:
                    ...
                    for k,v in matcher.groupdict().items(): #命名分组组成的字典
                          newdict[k] = translator[k](v) #将id 使用转换为int
                    request.vars = DictObj(newdict) #属性化使用

                    #response = handler(reqeust)
...
class MageWeb:
    ...
    def __call__(self, request:Request) ->Response :
        ...
        raise exc.HTTPNotFound('<h1>你访问的页面被劫持了</h1>')
